...
A GitHub Action that validates commit messages against project guidelines using [omni-dev](https://github.com/rust-works/omni-dev), either via AI-powered analysis (`check`) or deterministic, no-AI linting (`lint`).

## Features

- AI-powered commit message validation, or deterministic linting with no AI/API key required (requires omni-dev >= 0.40.0)
...
To use the deterministic linter instead (no API key needed):
...
| --------- | ------------------------------------------------------------------------------------------------ | ------- |
| `command` | omni-dev subcommand to run: `check` (AI-powered) or `lint` (deterministic, no AI/API key needed) | `check` |

`lint` requires omni-dev `>= 0.40.0` and ignores every AI-related input below (API keys, provider selection, `model`, `no-suggestions`, `batch-size`, and `guidelines` — it reads `.omni-dev/commit-rules.yaml` and `.omni-dev/scopes.yaml` instead).
...
*At least one API key is required for `command: check`, unless using Ollama. Not used by `command: lint`.
...
| Input         | Description                                                                       | Default         |
| ------------- | --------------------------------------------------------------------------------- | --------------- |
| `guidelines`  | Path to custom commit guidelines file (`command: check` only)                     | Auto-discovered |
| `context-dir` | Custom context directory for loading guidelines/rules/scopes (`check` and `lint`) | `.omni-dev/`    |
...
| `verbose`        | Show detailed analysis (model info for `check`; resolved rules/scopes for `lint`) | `false` |
| `show-passing`   | Include passing commits in output                                                 | `false` |
| `no-suggestions` | Skip generating corrected message suggestions (`command: check` only)             | `false` |

### Performance Configuration

| Input        | Description                                                         | Default |
| ------------ | ------------------------------------------------------------------- | ------- |
| `batch-size` | Number of commits to process per AI request (`command: check` only) | `4`     |
...
| Output            | Description                                             |
| ----------------- | ------------------------------------------------------- |
| `result`          | Full check/lint result in the specified format          |
...
| 3    | No commits found in range (`command: check` only — `lint` treats an empty range as clean) |
...
### Deterministic Linting (No API Key Required)
...
- At least one AI provider configured for `command: check` (not needed for `command: lint`, which requires omni-dev `>= 0.40.0`)
